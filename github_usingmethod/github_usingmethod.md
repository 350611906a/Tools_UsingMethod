...
2、在github上邀请共同协同协作的方法：

​		进入仓库 --> setting --> Manage access --> Invite a collaborater --> 选择合作人即可。

3、多方协作时候，后提交的若失败，处理方法如下：

-  多人处理不同文件的冲突：

  ​	git pull(先拉下来) --> q --> git push --> 会出现三方合并 ,若不想三方合并，使用git pull --rebase
...
- git hi时看到的origin/master为远程仓库所在位置
